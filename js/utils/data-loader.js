...
import resizeImage from "../game-data/resize";
...
export default class Loader {
  ...
  static preloadImages(data) {
    ...
    data.forEach((question) => {
      ...
      answers.forEach((answer) => {
        promises.push(
          loadImage(answer.src).then((image) => {
            ...
            const imageOptimizedSize = resizeImage(
              imageFrameSize,
              imageNaturalSize
            );
  ...
  static async saveResults({ answers, lives }, name = DEFAULT_NAME) {
    const result = {
      name,
      answers,
      lives
